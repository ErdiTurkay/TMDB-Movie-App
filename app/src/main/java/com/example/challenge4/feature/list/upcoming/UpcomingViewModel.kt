package com.example.challenge4.feature.list.upcoming
...
class UpcomingViewModel @Inject constructor(
    ...
    var upcomingMoviesLiveData = MutableLiveData<List<Movie>?>()
    ...
    fun getUpcomingMovies() {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                // Upcoming movies are pulled from the API.
                val response = api.getUpcomingMovies(page = page)
                val upcomingList = response.results
                ...
                upcomingMoviesLiveData.postValue(movieList)
    ...
    fun checkFavorite() {
        viewModelScope.launch(Dispatchers.IO) {
            // Favorite movies are pulled from the Room database.
            val favoriteIDs = db.movieDao().getAllIDs()
            val upcomingMoviesList = upcomingMoviesLiveData.value

            // Checked whether each movie in the list is added to favorites.
            if (upcomingMoviesList?.isNotEmpty() == true) {
                for (item in upcomingMoviesList) {
                    item.isFavorite = favoriteIDs.contains(item.id)
                }
            }

            // LiveData has been updated.
            upcomingMoviesLiveData.postValue(upcomingMoviesList)
        }
    }
